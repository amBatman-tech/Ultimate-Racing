using System.Collections;
using System.Collections.Generic;
using UnityEngine;

public class TrackFourAICar : MonoBehaviour {

	public GameObject TheMarker;
	public GameObject Mark00;
	public GameObject Mark01;
	public GameObject Mark02;
	public GameObject Mark03;
	public GameObject Mark04;
	public GameObject Mark05;
	public GameObject Mark06;
	public GameObject Mark07;
	public GameObject Mark08;
	public GameObject Mark09;
	public GameObject Mark10;
	public GameObject Mark11;
	public GameObject Mark12;
	public GameObject Mark13;
	public GameObject Mark14;
	public GameObject Mark15;
	public GameObject Mark16;
	public GameObject Mark17;
	public GameObject Mark18;
	public GameObject Mark19;
	public GameObject Mark20;
	public GameObject Mark21;
	public GameObject Mark22;
	public GameObject Mark23;
	public GameObject Mark24;
	public GameObject Mark25;
	public GameObject Mark26;
	public GameObject Mark27;
	public GameObject Mark28;
	public GameObject Mark29;
	public GameObject Mark30;
	public GameObject Mark31;
	public GameObject Mark32;
	public GameObject Mark33;
	public GameObject Mark34;
	public GameObject Mark35;
	public GameObject Mark36;
	public GameObject Mark37;
	public GameObject Mark38;
	public GameObject Mark39;
	public GameObject Mark40;
	public GameObject Mark41;
	public GameObject Mark42;
	public GameObject Mark43;
	public GameObject Mark44;
	public GameObject Mark45;
	public GameObject Mark46;
	public GameObject Mark47;
	public GameObject Mark48;
	public GameObject Mark49;
	public GameObject Mark50;
	public GameObject Mark51;
	public GameObject Mark52;
	public GameObject Mark53;
	public GameObject Mark54;
	public GameObject Mark55;
	public GameObject Mark56;
	public GameObject Mark57;
	public GameObject Mark58;
	public GameObject Mark59;
	public GameObject Mark60;
	public GameObject Mark61;
	public GameObject Mark62;
	public GameObject Mark63;
	public GameObject Mark64;
	public GameObject Mark65;
	public GameObject Mark66;
	public GameObject Mark67;
	public GameObject Mark68;
	public GameObject Mark69;
	public GameObject Mark70;
	public GameObject Mark71;
	public GameObject Mark72;
	public GameObject Mark73;
	public GameObject Mark74;
	public GameObject Mark75;
	public GameObject Mark76;
	public GameObject Mark77;
	public GameObject Mark78;
	public GameObject Mark79;
	public GameObject Mark80;
	public GameObject Mark81;


	public int MarkTracker;
	public GameObject AICar;

	void Update () {
		if (MarkTracker == 0) {
			TheMarker.transform.position = Mark00.transform.position;
		}
		if (MarkTracker == 1) {
			TheMarker.transform.position = Mark01.transform.position;
		}
		if (MarkTracker == 2) {
			TheMarker.transform.position = Mark02.transform.position;
		}
		if (MarkTracker == 3) {
			TheMarker.transform.position = Mark03.transform.position;
		}
		if (MarkTracker == 4) {
			TheMarker.transform.position = Mark04.transform.position;
		}
		if (MarkTracker == 5) {
			TheMarker.transform.position = Mark05.transform.position;
		}
		if (MarkTracker == 6) {
			TheMarker.transform.position = Mark06.transform.position;
		}
		if (MarkTracker == 7) {
			TheMarker.transform.position = Mark07.transform.position;
		}
		if (MarkTracker == 8) {
			TheMarker.transform.position = Mark08.transform.position;
		}
		if (MarkTracker == 9) {
			TheMarker.transform.position = Mark09.transform.position;
		}
		if (MarkTracker == 10) {
			TheMarker.transform.position = Mark10.transform.position;
		}
		if (MarkTracker == 11) {
			TheMarker.transform.position = Mark11.transform.position;
		}
		if (MarkTracker == 12) {
			TheMarker.transform.position = Mark12.transform.position;
		}
		if (MarkTracker == 13) {
			TheMarker.transform.position = Mark13.transform.position;
		}
		if (MarkTracker == 14) {
			TheMarker.transform.position = Mark14.transform.position;
		}
		if (MarkTracker == 15) {
			TheMarker.transform.position = Mark15.transform.position;
		}
		if (MarkTracker == 16) {
			TheMarker.transform.position = Mark16.transform.position;
		}
		if (MarkTracker == 17) {
			TheMarker.transform.position = Mark17.transform.position;
		}
		if (MarkTracker == 18) {
			TheMarker.transform.position = Mark18.transform.position;
		}
		if (MarkTracker == 19) {
			TheMarker.transform.position = Mark19.transform.position;
		}
		if (MarkTracker == 20) {
			TheMarker.transform.position = Mark20.transform.position;
		}
		if (MarkTracker == 21) {
			TheMarker.transform.position = Mark21.transform.position;
		}
		if (MarkTracker == 22) {
			TheMarker.transform.position = Mark22.transform.position;
		}
		if (MarkTracker == 23) {
			TheMarker.transform.position = Mark23.transform.position;
		}
		if (MarkTracker == 24) {
			TheMarker.transform.position = Mark24.transform.position;
		}
		if (MarkTracker == 25) {
			TheMarker.transform.position = Mark25.transform.position;
		}
		if (MarkTracker == 26) {
			TheMarker.transform.position = Mark26.transform.position;
		}
		if (MarkTracker == 27) {
			TheMarker.transform.position = Mark27.transform.position;
		}
		if (MarkTracker == 28) {
			TheMarker.transform.position = Mark28.transform.position;
		}
		if (MarkTracker == 29) {
			TheMarker.transform.position = Mark29.transform.position;
		}
		if (MarkTracker == 30) {
			TheMarker.transform.position = Mark30.transform.position;
		}
		if (MarkTracker == 31) {
			TheMarker.transform.position = Mark31.transform.position;
		}
		if (MarkTracker == 32) {
			TheMarker.transform.position = Mark32.transform.position;
		}
		if (MarkTracker == 33) {
			TheMarker.transform.position = Mark33.transform.position;
		}
		if (MarkTracker == 34) {
			TheMarker.transform.position = Mark34.transform.position;
		}
		if (MarkTracker == 35) {
			TheMarker.transform.position = Mark35.transform.position;
		}
		if (MarkTracker == 36) {
			TheMarker.transform.position = Mark36.transform.position;
		}
		if (MarkTracker == 37) {
			TheMarker.transform.position = Mark37.transform.position;
		}
		if (MarkTracker == 38) {
			TheMarker.transform.position = Mark38.transform.position;
		}
		if (MarkTracker == 39) {
			TheMarker.transform.position = Mark39.transform.position;
		}
		if (MarkTracker == 40) {
			TheMarker.transform.position = Mark40.transform.position;
		}
		if (MarkTracker == 41) {
			TheMarker.transform.position = Mark41.transform.position;
		}
		if (MarkTracker == 42) {
			TheMarker.transform.position = Mark42.transform.position;
		}
		if (MarkTracker == 43) {
			TheMarker.transform.position = Mark43.transform.position;
		}
		if (MarkTracker == 44) {
			TheMarker.transform.position = Mark44.transform.position;
		}
		if (MarkTracker == 45) {
			TheMarker.transform.position = Mark45.transform.position;
		}
		if (MarkTracker == 46) {
			TheMarker.transform.position = Mark46.transform.position;
		}
		if (MarkTracker == 47) {
			TheMarker.transform.position = Mark47.transform.position;
		}
		if (MarkTracker == 48) {
			TheMarker.transform.position = Mark48.transform.position;
		}
		if (MarkTracker == 49) {
			TheMarker.transform.position = Mark49.transform.position;
		}
		if (MarkTracker == 50) {
			TheMarker.transform.position = Mark50.transform.position;
		}
		if (MarkTracker == 51) {
			TheMarker.transform.position = Mark51.transform.position;
		}
		if (MarkTracker == 52) {
			TheMarker.transform.position = Mark52.transform.position;
		}
		if (MarkTracker == 53) {
			TheMarker.transform.position = Mark53.transform.position;
		}
		if (MarkTracker == 54) {
			TheMarker.transform.position = Mark54.transform.position;
		}
		if (MarkTracker == 55) {
			TheMarker.transform.position = Mark55.transform.position;
		}
		if (MarkTracker == 56) {
			TheMarker.transform.position = Mark56.transform.position;
		}
		if (MarkTracker == 57) {
			TheMarker.transform.position = Mark57.transform.position;
		}
		if (MarkTracker == 58) {
			TheMarker.transform.position = Mark58.transform.position;
		}
		if (MarkTracker == 59) {
			TheMarker.transform.position = Mark59.transform.position;
		}
		if (MarkTracker == 60) {
			TheMarker.transform.position = Mark60.transform.position;
		}
		if (MarkTracker == 61) {
			TheMarker.transform.position = Mark61.transform.position;
		}
		if (MarkTracker == 62) {
			TheMarker.transform.position = Mark62.transform.position;
		}
		if (MarkTracker == 63) {
			TheMarker.transform.position = Mark63.transform.position;
		}
		if (MarkTracker == 64) {
			TheMarker.transform.position = Mark64.transform.position;
		}
		if (MarkTracker == 65) {
			TheMarker.transform.position = Mark65.transform.position;
		}
		if (MarkTracker == 66) {
			TheMarker.transform.position = Mark66.transform.position;
		}
		if (MarkTracker == 67) {
			TheMarker.transform.position = Mark67.transform.position;
		}
		if (MarkTracker == 68) {
			TheMarker.transform.position = Mark68.transform.position;
		}
		if (MarkTracker == 69) {
			TheMarker.transform.position = Mark69.transform.position;
		}
		if (MarkTracker == 70) {
			TheMarker.transform.position = Mark70.transform.position;
		}
		if (MarkTracker == 71) {
			TheMarker.transform.position = Mark71.transform.position;
		}
		if (MarkTracker == 72) {
			TheMarker.transform.position = Mark72.transform.position;
		}
		if (MarkTracker == 73) {
			TheMarker.transform.position = Mark73.transform.position;
		}
		if (MarkTracker == 74) {
			TheMarker.transform.position = Mark74.transform.position;
		}
		if (MarkTracker == 75) {
			TheMarker.transform.position = Mark75.transform.position;
		}
		if (MarkTracker == 76) {
			TheMarker.transform.position = Mark76.transform.position;
		}
		if (MarkTracker == 77) {
			TheMarker.transform.position = Mark77.transform.position;
		}
		if (MarkTracker == 78) {
			TheMarker.transform.position = Mark78.transform.position;
		}
		if (MarkTracker == 79) {
			TheMarker.transform.position = Mark79.transform.position;
		}
		if (MarkTracker == 80) {
			TheMarker.transform.position = Mark80.transform.position;
		}
		if (MarkTracker == 81) {
			TheMarker.transform.position = Mark81.transform.position;
		}
	}


	IEnumerator OnTriggerEnter(Collider collision) {
		if (collision.gameObject.tag == "DreamCar01") {
			this.GetComponent<BoxCollider> ().enabled = false;
			MarkTracker += 1;
			if (MarkTracker == 82) {
				AICar.SetActive (false);
				//MarkTracker = 0;
			}
			yield return new WaitForSeconds (1);
			this.GetComponent<BoxCollider> ().enabled = true;
		}
	}
}
